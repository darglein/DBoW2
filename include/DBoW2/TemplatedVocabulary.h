...
template <class TDescriptor, class F>
TemplatedVocabulary<TDescriptor, F>& TemplatedVocabulary<TDescriptor, F>::operator=(
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::HKmeansStep(NodeId parent_id, const std::vector<pDescriptor>& descriptors,
                                                      int current_level)
{
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::initiateClustersKMpp(const std::vector<pDescriptor>& pfeatures,
                                                               std::vector<TDescriptor>& clusters) const
{
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::loadRaw(const std::string& file)
{
    BinaryFile bf(file, std::ios_base::in);
    bf >> m_k >> m_L >> m_scoring >> m_weighting;
    ...
    for (Node& n : m_nodes)
    {
        ...
        bf >> n.id >> n.parent >> n.weight >> n.word_id >> des;
        F::fromBinary(des, n.descriptor);
        m_nodes[n.parent].children.push_back(n.id);
    }
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::saveRaw(const std::string& file) const
{
    ...
    {
        typename F::BinaryDescriptor des;
        // the root doesn't have a descriptor
        if (n.id != 0) F::toBinary(n.descriptor, des);
        bf << n.id << n.parent << n.weight << n.word_id << des;
    }
    // words
    std::vector<std::pair<int, int>> words;
